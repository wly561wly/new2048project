...
public class Assess {
    ...
    private void check6(int[][]num,int[]p)
    {
        int isolatedFours = 0;  //检测孤立的4
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                if (num[i][j] == 4) {
                    int isIsolated = 0;
                    if (i > 0 && num[i - 1][j] == 4) isIsolated++;
                    if (i < 4 - 1 && num[i + 1][j] == 4) isIsolated++;
                    if (j > 0 && num[i][j - 1] == 4) isIsolated++;
                    if (j < 4 - 1 && num[i][j + 1] == 4) isIsolated++;
                    if (i > 0 && num[i - 1][j] ==0) isIsolated++;
                    if (i < 4 - 1 && num[i + 1][j]==0) isIsolated++;
                    if (j > 0 && num[i][j - 1] ==0) isIsolated++;
                    if (j < 4 - 1 && num[i][j + 1]==0) isIsolated++;
